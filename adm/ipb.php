...
if(isset($_GET['op']) && $_GET['op'] == 'i'){
    try{
		...
		$logError( 'we get json >>>> '.json_encode($config));//.'<br>';
		$logError( 'Check for tracling >>>> '.($config->trackingEnabled === true) .' >>>');
		$logError( 'Check for cookies >>>> '.(!isset($_COOKIE[$track->configUUID])) .' >>> '.(($_COOKIE[$track->configUUID])).' >>>>>>>' );
        $remoteIp = (get_client_ip_server() == 'NA'?get_client_ip_env():get_client_ip_server());
		$logError( 'Check for Remote IP >>> '.$remoteIp.' >>>>>>>' );
		if($config->trackingEnabled === true && (!isset($_COOKIE[$track->configUUID]))){
			//$_COOKIE[$track->configUUID] = $remoteIp;
            array_push($track->trackList, array(
                'time'=>date('d-m-Y H:i:s', time()), 
                'ip' => $remoteIp/*$_SERVER['REMOTE_ADDR']*/,
                'headers' => getallheaders()
            ));
			$logError( 'Update File >>>> [' .(__DIR__).'/'.$reportFolder.'/'.$trackUUID.'.json] >>>>>>>');//.'<br>';
            $track->time = time();
